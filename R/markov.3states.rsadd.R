...
if (min(length(times1),length(times2),length(sequences)) != max(length(times1),length(times2),length(sequences)))
        stop("Arguments 'times1', 'times2', and 'sequences' need to have the same length")
if (!all(is.na(times2[which(sequences==1 | sequences==13)])))		
	stop("Arguments 'times2' should be NA for right-censored individuals in X=1 or individuals who directly transited from X=1 to X=3")
...
if(max(p.age)<365.24)
 {warning("some ages at the baseline are very low (<1 year), check that 'p.age' is in days")} 
if (is.na(min(p.age)))
		warning("individuals with missing values on 'p.age' will be removed from the analysis \n")
...
if (is.na(min(p.sex)))
		warning("individuals with missing values on 'p.sex' will be removed from the analysis \n")
...
c12<-function(times1, times2, z12, z13, z23, cutimes12, cutimes13, cutimes23, H3P.times2)
 {return(log.h12(times1, z12, cutimes12) - H12(times1, z12, cutimes12) - H13(times1, z13, cutimes13) -H23(times1, times2, z23, cutimes23) - H3P.times2)}
...
c123<-function(times1, times2, z12, z13, z23, cutimes12, cutimes13, cutimes23, H3P.times2, h3P.times2)
 {return(log.h12(times1, z12, cutimes12) - H12(times1, z12, cutimes12) - H13(times1, z13, cutimes13) + log(exp(log.h23(times2, z23, cutimes23))+ h3P.times2) - H23(times1, times2, z23, cutimes23) - H3P.times2)}
...
.c12.H3P.times2<-H3P(rate.table=p.rate.table,age=.c12.p.age,sex=.c12.p.sex,year=.c12.p.year,.d12.2)
...
.c123.H3P.times2<-H3P(rate.table=p.rate.table,age=.c123.p.age,sex=.c123.p.sex,year=.c123.p.year,.d123.2)                                            
.c123.h3P.times2<-h3P(rate.table=p.rate.table,age=.c123.p.age,sex=.c123.p.sex,year=.c123.p.year,.d123.2)
