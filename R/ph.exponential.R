...
ph.exponential<- function(times, failures, group=NULL, cov.quanti=NULL, cov.quali=NULL, data, newdata=NULL, newtimes=NULL){
  ...
  # .flex.withoutcov=flexsurvreg(.f0, data = data,
  #                              dist = "exp", hessian = F)
  #
  # .flex<-flexsurvreg(.f, data = data,
  #                    dist = "exp", inits=c(exp(.flex.withoutcov$coefficients)),
  #                    hessian=F)
