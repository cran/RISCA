...
semi.markov.3states.rsadd <-function(times1, times2, sequences, weights=NULL, dist, cuts.12=NULL,cuts.13=NULL,cuts.23=NULL,ini.dist.12=NULL, ini.dist.13=NULL, ini.dist.23=NULL, cov.12=NULL, init.cov.12=NULL, names.12=NULL, cov.13=NULL, init.cov.13=NULL, names.13=NULL, cov.23=NULL, init.cov.23=NULL, names.23=NULL, p.age, p.sex, p.year, p.rate.table, conf.int=TRUE, silent=TRUE, precision=10^(-6))
{
...
if (!all(is.na(times2[which(sequences==1 | sequences==13)])))		
		stop("Arguments 'times2' should be NA for individuals right-censored in X=1 or individuals who directly transited from X=1 to X=3")
...
if (is.na(min(p.age)))
		warning("indivisuals with missing values for 'p.age' will be removed \n")
...
c1<-function(times1, z12, z13, cutimes12, cutimes13, H3P.times1)
 {return( -H12(times1, z12, cutimes12) - H13(times1, z13, cutimes13)- H3P.times1)}

c12<-function(times1, times2, z12, z13, z23, cutimes12, cutimes13, cutimes23, H3P.times2)
 {return(log.h12(times1, z12, cutimes12) - H12(times1, z12, cutimes12) - H13(times1, z13, cutimes13) -H23(times2, z23, cutimes23) - H3P.times2)}

c13<-function(times1, z12, z13, cutimes12, cutimes13, H3P.times1, h3P.times1)
 {return(log(exp(log.h13(times1, z13, cutimes13))+ h3P.times1) - H12(times1, z12, cutimes12) - H13(times1, z13, cutimes13) - H3P.times1)}

c123<-function(times1, times2, z12, z13, z23, cutimes12, cutimes13, cutimes23, H3P.times2, h3P.times2)
 {return(log.h12(times1, z12, cutimes12) - H12(times1, z12, cutimes12) - H13(times1, z13, cutimes13) + log(exp(log.h23(times2, z23, cutimes23))+ h3P.times2) - H23(times2, z23, cutimes23) - H3P.times2)}
...
.c1.H3P.times1<-H3P(rate.table=p.rate.table,age=.c1.p.age,sex=.c1.p.sex,year=.c1.p.year,.d1.1)
...
.c12.H3P.times2<-H3P(rate.table=p.rate.table,age=.c12.p.age,sex=.c12.p.sex,year=.c12.p.year,.d12.1+.d12.2)
...
.c13.H3P.times1<-H3P(rate.table=p.rate.table,age=.c13.p.age,sex=.c13.p.sex,year=.c13.p.year,.d13.1)
.c13.h3P.times1<-h3P(rate.table=p.rate.table,age=.c13.p.age,sex=.c13.p.sex,year=.c13.p.year,.d13.1)
...
.c123.H3P.times2<-H3P(rate.table=p.rate.table,age=.c123.p.age,sex=.c123.p.sex,year=.c123.p.year,.d123.1+.d123.2)                                            
.c123.h3P.times2<-h3P(rate.table=p.rate.table,age=.c123.p.age,sex=.c123.p.sex,year=.c123.p.year,.d123.1+.d123.2)
...
return( -1*(
 sum( .w1 * c1(.d1.1, .c1.12, .c1.13, cuts.12, cuts.13, .c1.H3P.times1) ) +
 sum( .w12 * c12(.d12.1, .d12.2, .c12.12, .c12.13, .c12.23, cuts.12, cuts.13, cuts.23, .c12.H3P.times2) ) +
 sum( .w13 * c13(.d13.1, .c13.12, .c13.13, cuts.12, cuts.13, .c13.H3P.times1, .c13.h3P.times1) ) +
 sum( .w123 * c123(.d123.1, .d123.2, .c123.12, .c123.13, .c123.23, cuts.12, cuts.13, cuts.23, .c123.H3P.times2, .c123.h3P.times2) ) ) )
}
